...
class UXAuditor:
    ...
    def audit_file(self, filepath: str) -> None:
        ...
        # --- 1. PSYCHOLOGY LAWS ---
        # Hick's Law
        clean_content = re.sub(r'<link\b[^>]*>', '', content, flags=re.IGNORECASE)
        nav_items = len(re.findall(r'<NavLink|<Link\b|<a\s+href|nav-item', clean_content, re.IGNORECASE))
        if nav_items > 7:
